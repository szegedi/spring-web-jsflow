...
public interface FlowStateIdGenerator {
    ...
    /**
     * Returns true if the return value of
     * {@link #generateStateId(NativeContinuation)} depends on the passed state
     * (that is, for a given state object, it will always return the same ID).
     * Implementations of the interface that implement counters or random
     * generators will return false. Implementations that use some information
     * contained in the continuation will return true. If an implementation
     * returns true, then returning an ID equal to an existing state's ID might
     * cause the user of the class to overwrite the existing ID. If an
