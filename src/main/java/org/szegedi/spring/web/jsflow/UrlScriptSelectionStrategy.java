...
public class UrlScriptSelectionStrategy implements ScriptSelectionStrategy {
     ...
     * then this strategy will always select a single script, the resource path
     * of its source file being specified in this property. If either servlet
     * path or path info (or both) are used, then the strategy will select
     * multiple scripts selected by servlet path and/or path info, and the path
     * specified here will be used as a common prefix for the resource paths of
     * script source files. Defaults to empty string, which is a handy value in
     * case no prefix is required and either servlet path or path info are used.
     * Be aware that this prefix can be further prefixed by a prefix specified
     * using {@link ScriptStorage#setPrefix(String)}) ScriptStorage class.
     * 
     * @param resourcePath
     */
